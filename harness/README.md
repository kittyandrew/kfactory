...
- OIDC device-flow login (`kfactory auth login`). The harness skips
...
- Nix with flakes enabled. The harness is invoked via `nix run`.
...
xdg-open http://localhost:8080/kfactory-harness  # or whatever opener
...
#    view at http://localhost:8080/kfactory-harness shows the messages.
...
## Configuration knobs (harness/configs/)

| File | Purpose |
|---|---|
| `opencode.json` | Plugin paths (substituted at image-build), permissive `permission` for dev. |
| `notification-ntfy.json` | ntfy plugin config: topic `kfactory-harness`, server `http://kfactory-ntfy:80`, `notifyAfter: 3` seconds on session.idle. Subscriber-aware cancellation is non-configurable and ALWAYS on -- there's no opt-out knob. The harness sees notifications fire because dev-test does not attach an SSE subscriber. |
...
pinned by digest yet because the harness is dev-only. To pin (and avoid
...
# Paste the @sha256:... into harness/scripts/dev-up.nix
```
...
harness/
  ...
  configs/
    opencode.json            -- opencode config (plugins + permission)
