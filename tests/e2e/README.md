...
- OIDC device-flow login (`kfactory auth login`). The e2e tests skips
...
- Nix with flakes enabled. The e2e tests is invoked via `nix run`.
...
|---|---|
| `nix run .#dev-up` | Build both OCI images, load into Docker, create network + volume, start ntfy + opencode + kfactory-cli. Waits for each to be healthy. |
...
```bash
# 1. Boot the e2e tests.
nix run .#dev-up
# ... waits for ntfy + opencode + kfactory-cli ...

# 2. Open ntfy in your browser BEFORE dispatching, so you see
#    notifications arrive in real time.
xdg-open http://localhost:8080/kfactory-e2e  # or whatever opener

# 3. Drop into the kfactory-cli container.
docker exec -it kfactory-cli bash

# Inside the container (everything is pre-configured):
$ kfactory auth status
kfactory auth status
  server:    http://kfactory-opencode:4096
  issuer:    http://e2e-fake-idp
  ...
...
$ docker exec kfactory-cli ls /root/.local/state/kfactory-loop/
...
#    view at http://localhost:8080/kfactory-e2e shows the messages.
...
## Configuration knobs (tests/e2e/configs/)

| File | Purpose |
|---|---|
| `opencode-base.json` | Non-plugin opencode config (permissive `permission` for dev). The `plugin` array is generated at evaluation time in `tests/e2e/opencode-image.nix` by iterating over `plugins.${system}` + `thirdPartyPlugins` (see flake.nix) and merged into this base. |
| `notification-ntfy.json` | ntfy plugin config: topic `kfactory-e2e`, server `http://kfactory-ntfy:80`, `notifyAfter: 3` seconds on session.idle. Subscriber-aware cancellation is non-configurable and ALWAYS on -- there's no opt-out knob. The e2e tests see notifications fire because dev-test does not attach an SSE subscriber. |
...
docker exec -it kfactory-cli kfactory attach <ref>
```

The bearer-and-routing patch logs the workspace ID it dispatches against
...
pinned by digest yet because the e2e tests are dev-only. To pin (and avoid
...
# Paste the @sha256:... into tests/e2e/scripts/dev-up.nix
```
...
tests/e2e/
  ...
  kfactory-cli-image.nix     -- builds kfactory-cli:dev OCI image
  ...
  scripts/
    ...
    dev-test.nix             -- scripted validation sequence
```
