...
## Configuration knobs (tests/regression/configs/)

| File | Purpose |
|---|---|
| `opencode-base.json` | Non-plugin opencode config (permissive `permission` for dev). The `plugin` array is generated at evaluation time in `tests/regression/opencode-image.nix` by iterating over `plugins.${system}` + `thirdPartyPlugins` (see flake.nix) and merged into this base. |
...
# Paste the @sha256:... into tests/regression/scripts/dev-up.nix
```
