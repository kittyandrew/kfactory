...
in
  pkgs.writeShellApplication {
    ...
    text = ''
      ...
      echo "[1/5] Loading Nix-built OCI images..."
      docker load < ${opencodeImage} 2>&1 | tail -1
      docker load < ${clientImage} 2>&1 | tail -1
      ...
      echo "[3/5] Starting ntfy + opencode + kfactory-client..."
      ...
      start_if_needed ${env.clientContainer} \
        -d --name ${env.clientContainer} \
        --network ${env.network} \
        ${env.images.client}

      echo "[4/5] Smoke check..."
      if docker exec ${env.clientContainer} kfactory list 2>&1 | grep -q "no workspaces\|^#"; then
        echo "      → kfactory list works against opencode"
      else
        echo "      → WARNING: kfactory list unexpected output"
        docker exec ${env.clientContainer} kfactory list || true
      fi
      ...
      echo "  │                                                            │"
      echo "  │  Drive kfactory from the client:                          │"
      echo "  │    docker exec -it ${env.clientContainer} bash            │"
